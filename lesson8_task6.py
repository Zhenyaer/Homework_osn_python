...
def inp():
    ...
    while flag:
        try:
            name = str(input('Введите наименование оргтехники: '))
            model = str(input('Введите модель оргтехники: '))
            cost = int(input('Введите стоимость модели: '))
            quantity = int(input('Введите количество модели: '))
            dop = str(input('Укажите дополнительную информацию: '))
            return [name, model, cost, quantity, dop]
        except ValueError:
            print('\nВы ввели некорректный тип данных. Введите данные заново:')


scan = inp()
scanner = Scanner(scan[0], scan[1], scan[2], scan[3], scan[4])
print(scanner)
